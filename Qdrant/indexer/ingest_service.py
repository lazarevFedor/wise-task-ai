...
from embedding_utils import EMBEDDING_MODEL, _get_embedding_model, _embed_query_text
...
from config_utils import _getenv_bool
from schemas import (
    PointIn,
    UpsertRequest,
    DeleteRequest,
...
def health() -> Dict[str, Any]:
    points_count = None
    try:
        r = requests.get(f"http://{QDRANT_HOST}:{QDRANT_PORT}/collections/{DEFAULT_COLLECTION}", timeout=3)
        if r.ok:
            js = r.json()
...
def embed_texts(body: EmbedRequest) -> Dict[str, Any]:
    ...
    if hasattr(model, 'embed'):  
        ...
        vectors = model.encode(body.texts, show_progress_bar=False, convert_to_numpy=True)
    try:
        import numpy as np
    ...
    if VECTOR_SIZE and size != VECTOR_SIZE:
        raise HTTPException(status_code=500, detail=f"Embedding dimension {size} != configured {VECTOR_SIZE}")
...
def collection_info(collection: str = DEFAULT_COLLECTION) -> Dict[str, Any]:
    try:
        r = requests.get(f"http://{QDRANT_HOST}:{QDRANT_PORT}/collections/{collection}", timeout=5)
        r.raise_for_status()
        js = r.json()
...
def search_simple(q: str, limit: int = 5, collection: Optional[str] = None, max_chars: int = 800) -> Dict[str, Any]:
    """Convenience GET endpoint: /v1/search-simple?q=...&limit=5"""
    req = SearchRequest(query=q, limit=limit, collection=collection, max_chars=max_chars)
...
def rag_simple(q: str, limit: int = 5, context_chars: int = 2000, collection: Optional[str] = None) -> Dict[str, Any]:
    """Convenience GET endpoint: /v1/rag-simple?q=...&limit=5&context_chars=2000"""
    req = RagRequest(query=q, limit=limit, context_chars=context_chars, collection=collection)
...
def auto_dispatch(body: AutoRequest) -> Dict[str, Any]:
    ...
    raise HTTPException(status_code=422, detail="Provide one of: points, delete_ids, texts, or query")
