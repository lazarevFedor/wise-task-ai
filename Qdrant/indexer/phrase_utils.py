...
def _expected_source_for_title(query: str) -> Optional[str]:
    ...
    if not q:
        return None
    if not re.search(r"[A-Za-zА-Яа-яЁё]", q):  # must contain letters
        return None
...
def _derive_phrases(query: str) -> List[str]:
    ...
    try:
        ...
        min_len = 6
    extra_sw = {s.strip().lower() for s in (os.getenv("PHRASE_STOPWORDS", "").split(",") if os.getenv("PHRASE_STOPWORDS") else []) if s.strip()}
    phrases: List[str] = []
    seen: set[str] = set()
